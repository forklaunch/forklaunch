...
import { {{pascal_case_name}}{{#is_worker}}Event{{/is_worker}}Record{{#is_worker}} as {{pascal_case_name}}EventRecordEntity{{/is_worker}} } from '../../persistence/entities/{{camel_case_name}}{{#is_worker}}Event{{/is_worker}}Record.entity';
...
export const {{pascal_case_name}}RequestMapper = requestMapper({
  ...
  entity: {{pascal_case_name}}{{#is_worker}}EventRecordEntity{{/is_worker}}{{^is_worker}}Record{{/is_worker}},
  mapperDefinition: {
    toEntity: async (dto{{^is_worker}}, em: EntityManager{{/is_worker}}) => {
      {{^is_worker}}return em.create({{pascal_case_name}}Record, {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
      } satisfies {{pascal_case_name}}EventRecord;{{/is_worker}}
    }
  }
});
...
export const {{pascal_case_name}}ResponseMapper = responseMapper({
  ...
  entity: {{pascal_case_name}}{{#is_worker}}EventRecordEntity{{/is_worker}}{{^is_worker}}Record{{/is_worker}},
  mapperDefinition: {
    toDto: async (entity: {{#is_worker}}{{pascal_case_name}}EventRecord{{/is_worker}}{{^is_worker}}InferEntity<typeof {{pascal_case_name}}Record>{{/is_worker}}) => {
