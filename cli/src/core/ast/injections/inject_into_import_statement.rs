...
pub(crate) fn inject_into_import_statement<'a>(
    ...
    let generated_comment_at_top = source_text.trim_start().starts_with("/**")
        && source_text.contains("Generated by ForkLaunch");

    if injection_pos == Some(0) && generated_comment_at_top {
