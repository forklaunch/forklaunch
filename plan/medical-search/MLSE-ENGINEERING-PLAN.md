...
| J. Hardening | Done (awaiting npm publish) | `README.md` for client developers (setup, API, HMAC backend-to-backend auth model with IAM permission guidance, safety model, privacy, licensed content, voice, AI cost, known limitations), linked into the CLI template. Scaffolded MLSE services now get `scripts/` and `eval/` plus `corpus:refresh`, `mesh:load` and `eval:run` scripts, checked in `init_mlse.sh`. Security review fixes: `GET /document/:id` applies the license and flag rules; case studies exclude licensed and flagged documents; topic numbers and case studies hide flagged documents; `GET /search` classifies before live retrieval (non-literature queries stay in the corpus, never reach live APIs), live cache keys are hashed, and queries are not logged; saved searches refuse patient-specific text; contract license strings count only through `LicensedContentAdapter`; passage markup escaped in the Claude prompt; per-call output cap 16k tokens; input limits on search sources and refresh terms. 123 unit + 48 service tests; fresh scaffold verified with the built CLI |
